...
public class AchievmentListener implements Listener {
    ...
    public void onEntityDeath(PlayerDeathEvent e) {
        try {
            ...
            boolean heart12 = LevelAPI.get12heart(k);
            boolean heart10 = LevelAPI.get10heart(k);
            boolean fundf = LevelAPI.getfundf(k);

            if (damageInFight.get(ent) >= 10.0){
                if (LevelAPI.getTank(ent) == false) {
                    LevelAPI.setTank(ent, true);
            ...
            if (ent instanceof Player && dc == EntityDamageEvent.DamageCause.FIRE || dc == EntityDamageEvent.DamageCause.FIRE_TICK && fundf == false) {
                LevelAPI.addXP(k, 20);
                ...
                inFight.remove(ent);
                LevelAPI.setfundf(ent, true);

            } else if (ent instanceof Player && ent.getHealth() == 24 && heart12 == false) {
                LevelAPI.addXP(k, 25);
                k.sendMessage("§aFalls du ein Level aufsteigen solltest und nicht aufgestiegen bist, warte ein paar Minuten");
                k.sendMessage("§aDu hast das Achievement §412 Heart §aerreicht! (Besiege einen Spieler indem er verbrennt)");
                inFight.remove(ent);
                LevelAPI.set12heart(ent, true);

            } else if (ent instanceof Player && ent.getHealth() == 20 && heart10 == false) {
                LevelAPI.addXP(k, 25);
                k.sendMessage("§aFalls du ein Level aufsteigen solltest und nicht aufgestiegen bist, warte ein paar Minuten");
                k.sendMessage("§aDu hast das Achievement §410 Heart §aerreicht! (Besiege einen Spieler indem er verbrennt)");
                inFight.remove(ent);
                LevelAPI.set10heart(ent, true);

            }
          if (LevelAPI.getnoob(ent) == false) {
          ...
          if (LevelAPI.gettryhard(k) == false && LevelAPI.getTerminator(k) == false) {
    ...
    public void onPlace(BlockPlaceEvent e) {
        ...
        if (LevelAPI.getSpiderman(e.getPlayer()) == false) {
            ...
            if (i == 10) {
                LevelAPI.setSpiderman(e.getPlayer(), true);
